...
{
    // 基本运算符重载
    friend BigDecimal operator+(const BigDecimal &, const BigDecimal &); // 加法重载
    friend BigDecimal operator-(const BigDecimal &, const BigDecimal &); // 减法重载
    friend BigDecimal operator*(const BigDecimal &, const BigDecimal &); // 乘法重载
    friend BigDecimal operator/(const BigDecimal &, const BigDecimal &); // 除法重载
    friend BigDecimal operator-(const BigDecimal &);                 // 负号重载

    // 比较重载
    friend bool operator==(const BigDecimal &, const BigDecimal &); // 等于重载
    friend bool operator!=(const BigDecimal &, const BigDecimal &); // 不等于重载
    friend bool operator<(const BigDecimal &, const BigDecimal &);  // 小于重载
    friend bool operator<=(const BigDecimal &, const BigDecimal &); // 小于等于重载
    friend bool operator>(const BigDecimal &, const BigDecimal &);  // 大于重载
    friend bool operator>=(const BigDecimal &, const BigDecimal &); // 大于等于重载

    // 扩展运算符重载
    friend BigDecimal operator+=(BigDecimal &, const BigDecimal &); // 加等重载
    friend BigDecimal operator-=(BigDecimal &, const BigDecimal &); // 减等重载
    friend BigDecimal operator*=(BigDecimal &, const BigDecimal &); // 乘等重载
    friend BigDecimal operator/=(BigDecimal &, const BigDecimal &); // 除等重载

    // 输入输出重载
    friend ostream &operator<<(ostream &, const BigDecimal &); // 输出重载
    friend istream &operator>>(istream &, BigDecimal &);       // 输入重载

public:
    BigDecimal();
    BigDecimal(int);                          // 用一个整数构造
    BigDecimal(double);                       // 用一个浮点数构造
    BigDecimal(const string &);               // 用一个字符串构造
    BigDecimal(const BigDecimal &);               // 用一个高精度数构造
    BigDecimal(BigDecimal &&) noexcept;           // 移动构造
    BigDecimal operator=(const BigDecimal &);     // 赋值函数
    BigDecimal operator=(BigDecimal &&) noexcept; // 移动赋值

    BigDecimal abs() const;      // 取绝对值
    BigDecimal pow(int n) const; // 幂运算
    ...
    {
        BigDecimal::ACCURACY = accuracy;
    }

#define BigDecimal_ZERO BigDecimal::ZERO()
#define BigDecimal_ONE BigDecimal::ONE()
#define BigDecimal_TEN BigDecimal::TEN()
...
inline BigDecimal::BigDecimal() // 默认构造函数
{
    tag = true;
    integer.push_back(0);
    decimal.push_back(0);
...
inline BigDecimal::BigDecimal(int num) // 用整型初始化
{
    if (num >= 0) // 判断正负
    {
        tag = true;
...
inline BigDecimal::BigDecimal(double num)
{
    *this = BigDecimal(std::to_string(num));
}

inline BigDecimal::BigDecimal(const string &num) // 用字符串初始化，格式形如"-123.456"、"1.0"
{
    // 用于判断小数与整数部分交界
...
inline BigDecimal::BigDecimal(const BigDecimal &num) // 利用高精度类初始化
{
    integer = num.integer;
    decimal = num.decimal;
    tag = num.tag;
...
inline BigDecimal::BigDecimal(BigDecimal &&num) noexcept // 移动构造
{
...
inline BigDecimal BigDecimal::operator=(const BigDecimal &num) // 赋值（拷贝）操作
{
    integer = num.integer;
    decimal = num.decimal;
    tag = num.tag;
...
inline BigDecimal BigDecimal::operator=(BigDecimal &&num) noexcept
{
    integer.swap(num.integer);
    decimal.swap(num.decimal);
    tag = num.tag;
...
inline BigDecimal BigDecimal::abs() const // 取绝对值
{
    if (tag)
        return (*this);
    else
...
inline BigDecimal BigDecimal::pow(int n) const
{
    BigDecimal ans = *this;
    for (int i = 1; i < n; i++)
    {
...
inline string BigDecimal::toString(size_t decimalNum) const
{
    string ans = "";
...
inline int BigDecimal::toInt() const // 转换为int
{
    return stoi(toString());
}

inline long BigDecimal::toLong() const // 转换为long
{
    return stol(toString());
}

inline long long BigDecimal::toLongLong() const // 转换为long long
{
    return stoll(toString());
}

inline float BigDecimal::toFloat() const // 转换为float
{
    return stof(toString());
}

inline double BigDecimal::toDouble() const // 转换为double
{
    return stod(toString());
}

inline long double BigDecimal::toLongDouble() const // 转换为long double
{
    return stold(toString());
}

inline BigDecimal operator-(const BigDecimal &num) // 取负操作
{
    BigDecimal temp(num);
    temp.tag = !temp.tag;
    return temp;
...
inline ostream &operator<<(ostream &out, const BigDecimal &num) // 输出重载
{
    if (!num.tag) // 负数
    {
        out << "-";
...
inline istream &operator>>(istream &in, BigDecimal &num) // 输入重载
{
    string str;
    in >> str;
    num = BigDecimal(str);
...
inline BigDecimal operator+=(BigDecimal &num1, const BigDecimal &num2) // 加等于重载
{
...
inline BigDecimal operator-=(BigDecimal &num1, const BigDecimal &num2) // 减等于重载
{
...
inline BigDecimal operator*=(BigDecimal &num1, const BigDecimal &num2) // 乘等于重载
{
    BigDecimal result(0);                               // 储存结果
    if (num1 == BigDecimal_ZERO || num2 == BigDecimal_ZERO) // 有0做乘数得0
...
inline BigDecimal operator/=(BigDecimal &num1, const BigDecimal &num2) // 除等于重载
{
    if (num2 == BigDecimal_ZERO)
        throw DividedByZeroException();
    if (num1 == BigDecimal_ZERO)
        return num1;
    if (num1 == num2)
        return (num1 = BigDecimal_ONE);

    BigDecimal temp_num1 = num1;
    BigDecimal temp_num2 = num2;
    ...
    else
    {
        ...
        while (t--)
        {
            temp_num1 = temp_num1 * BigDecimal_TEN;
            temp_num2 = temp_num2 * BigDecimal_TEN;
        }
    }
    ...
    int k = BigDecimal::ACCURACY;
    BigDecimal quotient(0); // 商
    ...
    while (k--)
    {
        ...
        {
            temp_num1 = temp_num1 * BigDecimal_TEN;
            quotient = quotient * BigDecimal_TEN;
        }
        else
        {
            int i;
...
inline BigDecimal operator+(const BigDecimal &num1, const BigDecimal &num2) // 调用+=
{
    BigDecimal temp(num1);
    temp += num2;
    return temp;
...
inline bool operator<(const BigDecimal &num1, const BigDecimal &num2) // 小于重载
{
    bool sign;                // 返回值
...
inline bool operator>(const BigDecimal &num1, const BigDecimal &num2) // 大于重载
{
    bool tag = !(num1 <= num2);
    return tag;
}

inline bool operator==(const BigDecimal &num1, const BigDecimal &num2) // 等于重载
{
...
inline bool operator!=(const BigDecimal &num1, const BigDecimal &num2)
{
    return !(num1 == num2);
}
