...
def create_lenet5(input_shape: tuple) -> models.Sequential:
    ...
    """
    mod = models.Sequential()  # 初始化序贯模型
    # 添加第一个卷积层，使用tanh激活函数，输入形状为input_shape，padding为same
    mod.add(layers.Conv2D(6, (5, 5), activation='tanh', input_shape=input_shape, padding='same'))
    # 添加第一个平均池化层
    mod.add(layers.AveragePooling2D((2, 2), strides=2))
    # 添加第二个卷积层，使用tanh激活函数，padding为valid
    mod.add(layers.Conv2D(16, (5, 5), activation='tanh', padding='valid'))
    # 添加第二个平均池化层
    mod.add(layers.AveragePooling2D((2, 2), strides=2))
    # 添加第三个卷积层，使用tanh激活函数，padding为valid
    mod.add(layers.Conv2D(120, (5, 5), activation='tanh', padding='valid'))
    # 展平层，为全连接层做准备
    mod.add(layers.Flatten())
    # 添加第一个全连接层，使用tanh激活函数
    mod.add(layers.Dense(84, activation='tanh'))
    # 添加输出层，使用softmax激活函数，输出10类概率
    mod.add(layers.Dense(10, activation='softmax'))
    mod.summary()  # 打印模型结构摘要
    return mod

model = create_lenet5( (32, 32, 1) )
...
def train_model(models, train_image, train_label, test_image, test_label, epochs=15):
    """
    训练给定模型的函数。
