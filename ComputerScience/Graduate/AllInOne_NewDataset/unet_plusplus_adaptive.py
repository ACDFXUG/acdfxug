...
class AdaptiveModulationLayer(nn.Module):
    ...
    def forward(self, features, deg_embedding):
        ...
        sigma = torch.sqrt(var + 1e-5)  # 避免除零    
...
class UNetPlusPlusAdaptive(nn.Module):
    ...
    def forward(self, x):
        ...
        # 2. Encoder
        x0_0 = self.enc0(x)           # 保持原始尺寸
        x1_0 = self.enc1(self.pool(x0_0))   # 尺寸减半
        x2_0 = self.enc2(self.pool(x1_0))   # 尺寸变为1/4
        
        # 3. Center + 自适应调制
        x3_0 = self.center_conv(self.pool(x2_0))  # 尺寸变为1/8
...
if __name__ == "__main__":
    ...
    x = torch.rand(2, 1, 362, 362)  # 测试362x362的输入
    y = model(x)
