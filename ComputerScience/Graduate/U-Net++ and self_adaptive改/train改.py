...
def get_severity_distribution(epoch):
    """根据epoch返回退化程度的概率分布"""
    if epoch < 15:
        # 早期：主要light，少量medium/heavy
        return {'light': 0.70, 'medium': 0.25, 'heavy': 0.05}
    elif epoch < 35:
        # 中期：主要medium，混合light/heavy
        return {'light': 0.20, 'medium': 0.60, 'heavy': 0.20}
    else:
        # 后期：均衡混合所有程度
        return {'light': 0.15, 'medium': 0.35, 'heavy': 0.50}
...
def main():
    ...
    print(f"   - unet_best.pth (总体最佳)")
    print(f"   - unet_final.pth (最终模型)")
