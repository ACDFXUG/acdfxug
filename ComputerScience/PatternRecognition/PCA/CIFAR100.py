...
def create_visualizations(data, labels, class_names, prefix, save_dir='images', max_samples_per_class=100):
    """
    ...
    :param max_samples_per_class: 每个类别最多绘制的样本数量。
    """
    os.makedirs(save_dir, exist_ok=True)

    # 将 labels 转换为 numpy 数组以便索引
    labels = np.array(labels)

    # --- 核心修改：筛选数据，每个类别最多 max_samples_per_class ---
    selected_indices = []
    for class_id in range(len(class_names)):
        # 找到当前类别所有样本的索引
        class_indices = np.where(labels == class_id)[0]
        # 选择前 max_samples_per_class 个索引
        selected_indices.extend(class_indices[:max_samples_per_class])

    # 按照筛选出的索引获取数据和标签
    data_subset = data[selected_indices]
    labels_subset = labels[selected_indices]

    print(f"Subset - {prefix}: Total samples selected = {len(selected_indices)} "
          f"(Max {max_samples_per_class} per class)")
    ...
    plt.figure(figsize=(10, 8))
    scatter = plt.scatter(data_2d[:, 0], data_2d[:, 1], c=labels_subset, cmap='tab10', s=10)
    plt.title(f'PCA Visualization - {prefix} (Max {max_samples_per_class}/class)')
    plt.xlabel('Principal Component 1')
    plt.ylabel('Principal Component 2')
    plt.legend(handles=scatter.legend_elements()[0], labels=class_names, loc='upper right')
    plt.savefig(os.path.join(save_dir, f'{prefix}_pca_max{max_samples_per_class}.png'), dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.figure(figsize=(10, 8))
    scatter = plt.scatter(data_2d[:, 0], data_2d[:, 1], c=labels_subset, cmap='tab10', s=10)
    plt.title(f't-SNE Visualization - {prefix} (Perplexity=30, Max {max_samples_per_class}/class)')
    plt.xlabel('t-SNE Dimension 1')
    plt.ylabel('t-SNE Dimension 2')
    plt.legend(handles=scatter.legend_elements()[0], labels=class_names, loc='upper right')
    plt.savefig(os.path.join(save_dir, f'{prefix}_tsne_perp30_max{max_samples_per_class}.png'), dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.figure(figsize=(10, 8))
    scatter = plt.scatter(data_2d[:, 0], data_2d[:, 1], c=labels_subset, cmap='tab10', s=10)
    plt.title(f't-SNE Visualization - {prefix} (Perplexity=50, Max {max_samples_per_class}/class)')
    plt.xlabel('t-SNE Dimension 1')
    plt.ylabel('t-SNE Dimension 2')
    plt.legend(handles=scatter.legend_elements()[0], labels=class_names, loc='upper right')
    plt.savefig(os.path.join(save_dir, f'{prefix}_tsne_perp50_max{max_samples_per_class}.png'), dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    # 5. 为所有数据集生成可视化图 (现在每个类别最多100个点)
    print("\n--- Generating visualizations for Original data (Max 100/class) ---")
    # 为原始数据生成可视化 (每个类别最多100个点)
    create_visualizations(data, labels, class_names, "Original", max_samples_per_class=100)

    print("\n--- Generating visualizations for Noisy data (Max 100/class) ---")
    # 为加噪数据生成可视化 (每个类别最多100个点)
    create_visualizations(noisy_data, labels, class_names, "Noisy", max_samples_per_class=100)

    # 为降噪数据生成可视化 (这里我们选择 n_components=100 的结果作为代表)
    denoised_data_for_vis = denoised_dict[100]  # 选择100个主成分的结果
    print("\n--- Generating visualizations for Denoised (n=100) data (Max 100/class) ---")
    create_visualizations(denoised_data_for_vis, labels, class_names, "Denoised_n100", max_samples_per_class=100)
